	...
	PNPController.viewHome = (req, res) => {
		 ...
		 database.task(task => {
            ...
            for (var i = 0; i < homeData[0].length; i++){
                ...
                if (homeData[0][i].publicitystatus == 0) {
                    uncheckedPublicityMaterials = uncheckedPublicityMaterials + 1;
                }
                else if(homeData[0][i].publicitystatus == 1){
            ...
            return database.task(task => {
                return task.batch([
                    ...
                    Promise.resolve(uncheckedPublicityMaterials),
                    Promise.resolve(ApprovedPublicityMaterials),
                    Promise.resolve(PendedPublicityMaterials),
            ...
            let uncheckedPublicityMaterials = homeData.shift();
            let ApprovedPublicityMaterials = homeData.shift();
            let PendedPublicityMaterials = homeData.shift();
            ...
            let uncheckedPublicityMaterials = organizationDatas.shift();
            let ApprovedPublicityMaterials = organizationDatas.shift();
            let PendedPublicityMaterials = organizationDatas.shift();
        ...
        }).then(([organizations, gosm, uncheckedPublicityMaterials, ApprovedPublicityMaterials, PendedPublicityMaterials, totalGrade]) => {
            let renderData = Object.create(null);
            ...
            logger.debug('Rendering PNP/mainPNP', log_options);
