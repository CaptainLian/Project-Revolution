SELECT *
  FROM ((SELECT PADP."GOSMActivity" as gosmactivity, 0 as transaction, to_char(PP.ACTUALDATESTART, 'MonthDD, YYYY') AS startdate, PADP.amount as amount, PADP.status
...
UNION (SELECT PACA."GOSMActivity" as gosmactivity, 1 as transaction, to_char(PP.ACTUALDATESTART, 'MonthDD, YYYY') AS startdate, SUM(PPE.quantity*PPE.unitcost) as amount, PACA.status
	 ...
	 GROUP BY PACA."GOSMActivity", PP.ACTUALDATESTART, PACA.STATUS)	) U;