...
--                      WHERE STATUS = 0 )
--  GROUP BY GA.ID
SELECT PP.GOSMACTIVITY AS ID, GA.STRATEGIES, AV.NAME, TO_CHAR(PP.ACTUALDATESTART,'Mon DD, YYYY') AS DATE, PP.VENUE as VENUEID, TO_CHAR(PPP.DATE+PPP.STARTTIME,'HH12:MI AM') AS TIME 
  FROM PROJECTPROPOSAL PP  JOIN GOSMACTIVITY GA
                            ON PP.GOSMACTIVITY = GA.ID
                   JOIN "ActivityVenue" AV                  
                            ON PP.VENUE = AV.ID
